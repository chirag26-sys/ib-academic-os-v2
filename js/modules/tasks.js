async function renderTasks(main) {
  const [tasks, subjects] = await Promise.all([DB.Store.getAll("tasks"), DB.Store.getAll("subjects")]);
  ...
  main.innerHTML = `
    ...
    ${open.length ? open.map((t) => taskCard(t, subjectName(t.subjectId), false)).join("") : `<div class="empty">Nothing open. Add your first task above.</div>`}

    ${done.length ? `<h2 class="mt">Done (${done.length})</h2>${done.map((t) => taskCard(t, subjectName(t.subjectId), true)).join("")}` : ""}
  `;
...
function taskCard(t, subjectName, isDone) {
  ...
  return `
    <div class="card ${isDone ? "" : p.toLowerCase()}">
      ...
      <div class="muted">${subjectName}${t.durationMin ? ` · ~${t.durationMin}min` : ""}</div>
      <div class="mt">
