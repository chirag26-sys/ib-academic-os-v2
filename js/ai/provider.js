...
async function buildWhatShouldIDoPrompt() {
  const [tasks, assessments, subjects] = await Promise.all([
  ...
  const taskLines = openTasks
    .map((t) => {
      const p = Priority.computePriority(t, assessmentById(t.assessmentId));
      return `- [${p}] ${t.title} (${subjectName(t.subjectId)}, due ${t.dueDate || "no date"}, ~${t.durationMin || "?"}min${t.assessmentId ? `, linked assessment: ${assessmentById(t.assessmentId)?.title || "unknown"}` : ""})`;
    })
